...
def select_model(profile, fov, alpha):
    if alpha == 0.06:
        ...
        nside = 1024
    M87_filename    = "../output/decay_M87_{}2D_FOVdiameter{:.1f}deg_rse1_alphaint{:.2f}deg_nside{}.fits".format(profile, fov, alpha, nside)
    M49_filename    = "../output/decay_M49_{}2D_FOVdiameter{:.1f}deg_rse1_alphaint{:.2f}deg_nside{}.fits".format(profile, fov, alpha, nside)
    VirgoC_filename = "../output/decay_VirgoC_{}2D_FOVdiameter{:.1f}deg_rse1_alphaint{:.2f}deg_nside{}.fits".format(profile, fov, alpha, nside)
...
def generate_cartesian_file(hdus, lon, lat, label, index=1):
    ...
    hdulist.writeto('../templates/{}_Dfactor_template.fits'.format(label))
    f = open('../templates/{}_Dfactor_template.txt'.format(label), "w")
    f.write("For {}: Dmax={}".format(label, np.log10(Jtotal.max())))

"""
M87_hdus, M49_hdus, Virgo_hdus = select_model("GAO", 10.0, 0.11)
...
"""
M87_hdus, M49_hdus, Virgo_hdus = select_model("B01", 10.0, 0.11)

generate_cartesian_file(M87_hdus, lon_M87, lat_M87, "M87_B01")
generate_cartesian_file(M49_hdus, lon_M49, lat_M49, "M49_G01")
