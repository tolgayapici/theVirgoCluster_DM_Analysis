...
def plot_for_model(model):
    filenames = glob.glob("../results/annihilation_*channel4_{}.log".format(model))
    masses  = []
    results = []
...
def plot_dSph_results():
    combined_filename = "/home/tyapici/Projects/HAWC_projects/HAWC_dSph_DarkMatter/results/txt/annihilation/Combined_wo_TriII_tautau.dat"
    masses, results = np.loadtxt(combined_filename, unpack=True)
    plt.loglog(masses/2., results, label=r'dSph w/o TriII $\tau^+\tau^-$')
    
    combined_filename = "/home/tyapici/Projects/HAWC_projects/HAWC_dSph_DarkMatter/results/txt/annihilation/Combined_w_TriII_tautau.dat"
    masses, results = np.loadtxt(combined_filename, unpack=True)
    plt.loglog(masses/2., results, label=r'dSph w/ TriII $\tau^+\tau^-$')
